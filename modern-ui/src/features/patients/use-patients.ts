...
import type { PatientsResponse, PatientsFilters as Filters } from "./type";
...
export const usePatients = (props?: Props) => {
    ...
    const { object: filters, setMultipleValues: setFilters } =
        useObject<Filters>({
            ...
            providerId: defaultFilters?.providerId || undefined,
            ...defaultFilters,
        });

    const listQuery = useQuery({
        queryKey: [queryKeys.patients, ...Object.values(filters)],
        queryFn: () => {
            return api<PatientsResponse>(endpoints.api.patients, {
                method: "GET",
            });
        },
