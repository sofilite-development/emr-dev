...
<script type="text/html" id="menu-action">
    ...
    <div class='menuLabel px-3' data-bind="text:label,click: menuActionClick,css: {menuDisabled: ! enabled()}"></div>
</script>
<script type="text/html" id="menu-header">
    <i data-bind="css: icon" class="fa closeButton"></i>
    <div class="menuSection dropdown">
        <div class='menuLabel px-3 dropdown-toggle oe-dropdown-toggle' data-bind="text:label" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></div>
...
<script type="text/html" id="menu-template">
    <div class='appMenu navbar-nav mr-auto' data-bind="foreach: menu">
        <div data-bind="template: {name:header ? 'menu-header' : 'menu-action', data: $data }" class="nav-item"></div>
    </div>
</script>
