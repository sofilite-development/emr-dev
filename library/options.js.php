...
<script>
    ...
    function checkSkipConditions() {
        ...
        for (var i = 0; i < skipArray.length; ++i) {
            ...
            if (is_multiple) {
                switch (operator) {
                    case 'eq':
                        condition = (-1 !== elem_val.indexOf(value));
                        break;
                    case 'ne':
                        condition = (-1 == elem_val.indexOf(value));
                        break;
                    case 'se':
                        condition = srcelem.checked;
                ...
                if (operator == 'eq') condition = elem_val == value;
                else
                if (operator == 'ne') condition = elem_val != value;
                else
                if (operator == 'se') condition = srcelem.checked;
                else
            ...
            if (prevandor == 'and') condition = condition && prevcond;
            else
    ...
    function sel_patient(ename, epid) {
        ...
        let newUrl = "/interface/main/calendar/find_patient_popup.php";
        if(epid?.name === "form_guardianid"){
            newUrl+= "?is_child=no"
        }
        dlgopen(`<?php echo $GLOBALS['webroot']; ?>${newUrl}`, '_blank', 500, 400);
    }
